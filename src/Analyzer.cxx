  ...
  while (attenuationLength <= 40)
  {
    ...
    while ( radius <= m_diskRadius ) 
    {
      ...
      while ( theta < 360 ) 
      {
        ...
        if ( likelihood > m_likelihood) { m_likelihood = likelihood; m_N0 = N0; m_radius = radius; m_theta = theta; m_attenuationLength = attenuationLength; } 
...
void Analyzer::RunReco(SiPMToTriggerMap& sipmToTriggerMap, const SiPMInfoMap& sipmInfoMap, const Configuration& config, const unsigned& trigger)
{
  ...
  m_likelihood    = std::numeric_limits<float>::lowest();
  m_radius = 0; m_theta = 0; m_N0 = 0; m_attenuationLength = 0;
  m_data.clear();
